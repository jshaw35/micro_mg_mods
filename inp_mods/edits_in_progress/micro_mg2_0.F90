...
use phys_grid,        only: get_rlat_all_p  !zsm
...
subroutine micro_mg_tend ( &
  ...
  real(r8), intent(in) :: mgrlats(mgncol)! latitudes in radians !zsm
  ...
  real(r8), intent(out) :: sadliq(mgncol,nlev)       ! cloud ice surface area density (cm2/cm3) !zsm
...
!        if (mgrlats(i)*180._r8/3.14159_r8.gt.+66.66667_r8) wbfeffmult(i,k) = 2._r8
     ...
     bergs(:,k)=bergs(:,k)*micro_mg_berg_eff_factor*wbfeffmult(:,k) !zsm
     ...
     if (do_cldice) then
        ...
        berg(:,k)=berg(:,k)*micro_mg_berg_eff_factor*wbfeffmult(:,k) !zsm
...
subroutine micro_mg_get_cols(lchnk, ncol, nlev, top_lev, qcn, qin, & !zsm
  ...
  call get_rlat_all_p(lchnk, ncol, rlats) !get latitudes !zsm
